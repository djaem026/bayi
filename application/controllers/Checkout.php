...
class Checkout extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $this->render('checkout', $head, $data);
            ...
            $this->session->set_flashdata('userError', "Please login or create account!");
    ...
    private function sendNotifications()
    {
        ...
        if (!empty($users)) {
            ...
            foreach ($users as $user) {
                $this->do_email($user, 'Admin', 'New order in ' . $myDomain, 'Hello, you have new order. Can check it in /admin/orders');
            }
        }
    }
    ...
    private function setActivationLink()
    {
        if ($this->config->item('send_confirm_link') === true) {
            ...
            if ($result == true) {
                ...
                $this->do_email($_POST['email'], $_POST['first_name'] . ' ' . $_POST['last_name'], lang('confirm_order_subj') . $url['host'], $msg);
            }
        }
    }
    ...
    public function paypal_success()
    {
    	...
    	function do_email($to , $recipientName , $subject, $msg)

	{
		...
		$this->email->to($to , $recipientName);
